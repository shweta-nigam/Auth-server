...
app.use(async (req, res, next) => {
    ...
    if (!token)
        return next();
    try {
        const decoded = jsonwebtoken_1.default.verify(token, cert_1.PUBLIC_KEY, {
            algorithms: ["RS256"],
        });
...
app.post("/o/authenticate/sign-in", async (req, res) => {
    ...
    // ✅ FIX: proper redirect (not JSON)
    if (client_id && redirect_uri) {
        return res.redirect(`/o/authorize?client_id=${encodeURIComponent(client_id)}&redirect_uri=${encodeURIComponent(redirect_uri)}`);
    }
...
app.get("/o/userinfo", async (req, res) => {
    ...
    const [user] = await db_1.db
        .select()
        .from(schema_1.usersTable)
        .where((0, drizzle_orm_1.eq)(schema_1.usersTable.id, userClaims.sub))
        .limit(1);
    if (!user) {
    ...
    res.json({
        ...
        email_verified: user.emailVerified,
        given_name: user.firstName,
        family_name: user.lastName, // ✅ fixed
        name: [user.firstName, user.lastName].filter(Boolean).join(" "),
        picture: user.profileImageURL,
    });
});
...
app.post("/o/authorize/confirm", async (req, res) => {
    ...
    const [appData] = await db_1.db
        ...
        .limit(1);
    if (!appData || appData.redirectUri !== redirect_uri) {
        return res.status(400).json({ message: "Invalid client or redirect_uri" });
    }
...
app.post("/o/token", async (req, res) => {
    ...
    const [appData] = await db_1.db
        ...
        .where((0, drizzle_orm_1.eq)(schema_1.applicationsTable.clientId, client_id))
        .limit(1);
    if (!appData || appData.clientSecret !== client_secret) {
        return res.status(401).json({ message: "Invalid client" });
    }
    // 2. Validate redirect_uri (using applicationsTable)
    if (appData.redirectUri !== redirect_uri) {
...
//  for home of 
app.post("/o/register-client", async (req, res) => {
    try {
        // const { appName, redirectUri } = req.body;
        const { name, redirect_uri } = req.body;
        console.log("BODY:----------------------------------------------", req.body);
        const client_id = node_crypto_1.default.randomUUID();
        const client_secret = node_crypto_1.default.randomUUID();
        await db_1.db.insert(schema_1.applicationsTable).values({
            name,
            clientId: client_id,
